...
2. **遭遇战斗：** 挂机时生成敌人 → 双方血条可见 → 自动互相攻击。势均力敌约 **5 分钟**（约 240–360 秒）的缓慢消耗战，而非秒杀。低级怪仍较弱、掉落以废料为主，但也不会 3 秒融化；高等级怪更硬、掉落更好。战后短喘息（远短于一场战斗）。
3. **物品技能 + 三格技能栏：** 部分装备定义 `skill`（伤害 / 治疗 / 护盾）。**点空位**打开选择器，从当前装备解锁的技能里装配；**点已装配**可清空或重选。独立冷却清晰显示；自动/手动施放时有可见特效（腕刃斩击闪光、治疗脉冲、护盾微光、酸蚀喷溅）。
...
4. 装备带 ★ 的物品后，点技能栏空位 → 弹出选择器 → 点技能名装配。再点该格可清空/更换。冷却显示在格子上；就绪后战斗中自动释放，并播放对应特效。
